...
def draw_crosshair_and_mark_contours(frame, contours, crosshair_color=(0, 0, 255), contour_color=(255, 0, 0)):
    ...
    for contour in contours:
        ...
        if M['m00'] != 0:
            ...
            cv2.circle(frame, (cX, cY), 5, contour_color, -1)

            # Подписываем координаты на изображении
            cv2.putText(frame, f"({relative_x},{relative_y})", (cX + 10, cY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, contour_color, 2)

    return frame

def main():
    # Открываем видеопоток с камеры (по умолчанию - первая камера в системе)
    ...
    while True:
        ...
        # Пытаемся получить GPS координаты с таймаутом
        latitude, longitude = get_gps_coordinates(gps_serial, timeout=1)

        # Обнаружение белых контуров на кадре
        frame, contours = detect_white_shapes(frame)
        ...
        if latitude is not None and longitude is not None:
            relative_coords = find_corner(frame, contours, latitude, longitude)
        else:
            relative_coords = []

        # Отображаем GPS координаты на изображении
        frame = display_image_with_gps(frame, latitude, longitude)
