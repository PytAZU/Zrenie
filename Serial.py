...
def get_gps_coordinates(port='COM6', baudrate=115200, timeout=1):
    # Подключаемся к последовательному порту GPS-модуля
...
def find_corner(image, contours, center_lat, center_lon):
...
def save_coordinates_to_file(center_lat, center_lon, relative_coords, output_dir="Coordinates"):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    for i, (rel_x, rel_y) in enumerate(relative_coords):
        file_path = os.path.join(output_dir, f"contour_{i + 1}_coordinates.txt")
        with open(file_path, 'w') as file:
            file.write(f"Center GPS Coordinates: Latitude = {center_lat}, Longitude = {center_lon}\n")
            file.write(f"Relative Coordinates:\n")
            file.write(f"X: {rel_x}, Y: {rel_y}\n")
...
def main():
    ...
    relative_coords = find_corner(output_image, contours, latitude, longitude)
